...
export default function Orb({
  ...
  useEffect(() => {
    ...
    const update = (time: number) => {
      ...
      program.uniforms.backgroundColor.value = colorToVec3(backgroundColor);
  ...
  return (
    <div ref={containerRef} className={cn('orb-container relative h-full w-full overflow-hidden', className)} />
  );
}
...
function colorToVec3(color: string) {
  ...
  if (hslMatch) {
    const [, hValue = '0', sValue = '0', lValue = '0'] = hslMatch;
    const h = Number.parseInt(hValue, 10) / 360;
    const s = Number.parseInt(sValue, 10) / 100;
    const l = Number.parseInt(lValue, 10) / 100;
